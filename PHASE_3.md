...
| 3.8 | Logout mid-party handling | 🟢 Complete | When a party member disconnects, auto-submit their vote using the event's `offlineFallback` strategy IF the other party members have ALL voted starting a five minute timer and it passed; remove from participants so `allVoted()` recalculates |
| 3.9 | End-to-end multiplayer test | 🟢 Complete | Two `runClient` instances; same event instance; vote on a choice; verify both clients advance to the same scene; test mid-event disconnect |
